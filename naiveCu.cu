...
__global__ void coolKer(cuda::std::complex<float> *res, const int cols){
    ...
    for(int s=1; s<=cuda::ilog2(cols); s++){
        int m = 1 << s;
        cuda::std::complex<float> wm = cuda::std::polar(1.f, -2 * cuda::std::numbers::pi_v<float> / (float) m);
...
__global__ void coolKerV1(cuda::std::complex<float> *res, const int cols){
    ...
    for(int s=1; s<=cuda::ilog2(cols); s++){
        ...
        for(int l=0; l<cols/2; l++){
            ...
            int k = (2 * l >> cuda::ilog2(m)) * m;
            cuda::std::complex<float> w = cuda::std::polar(1.f, -2 * j * cuda::std::numbers::pi_v<float> / (float) m);

            cuda::std::complex<float> t = w * res[tId * cols + k + j + (m >> 1)];
            cuda::std::complex<float> u = res[tId * cols + k + j];
            res[tId * cols + k + j] = u + t;
            res[tId * cols + k + j + (m >> 1)] = u - t;
        }
    }
}
...
//    const int blocks = rows / threadsCool;
    ...
    coolKer<<<blocks, threadsCool, 0, stream>>>(DgridT, cols);
...
//    transposeKer<<<blocksT, threadsXBlockT, 0, stream>>>(DgridT, Dgrid, cols);
    ...
    if (saveData){
        ...
        save.open("data/new.bin", std::ios::binary);
